...
- antagonist #2: **three P1 completion routes, none ready**. (1) Wen et al. 2024, *Parnassia wightiana × Nonarthra variabilis*/florivorous beetles: early/middle/late cohorts are explicitly ordered by contemporaneous beetle abundance (0%, <20%, >70% flowers), and final seed production is measured; the full raw dataset is public in Dryad, but the paper reports seeds per surviving fruit while beetle peduncle damage removed many marked flowers, so the raw fate table must be audited before defining a net final-reproduction SMD. (2) Jordano et al. 1990, *Astragalus lusitanicus × Tomares ballus*: timing and final ripe-fruit reproduction are present but the published RSI variance is at the wrong nested grain. (3) James 1998, *Yucca kanabensis × non-pollinating Tegeticula* cheater: plant-level timing and final seed damage were both measured, but timing-stratified final-seed summaries are not published;
