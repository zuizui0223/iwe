...
- antagonist #2: **two P1 one-blocker routes, neither ready**. (1) Jordano et al. 1990, *Astragalus lusitanicus × Tomares ballus*: weekly egg laying and immature-inflorescence availability directly establish the partner window and final ripe-fruit reproduction is reported, but the published RSI variance is at an inflorescence/nested grain incompatible with the patch-level synchrony contrast; plant/shoot-level variance is still required. (2) James 1998, *Yucca kanabensis × non-pollinating Tegeticula* cheater: the open thesis directly measures same-season antagonist emergence versus host flowering/fruit age and later counts intact versus damaged seeds per plant, but Chapter 4 publishes timing effects only for oviposition/larval use rather than early-versus-late final seed means and variance. Unpublished plant-level seed-damage summaries would unlock an SMD;
