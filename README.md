...
- mixed #2: **two complementary P1 leads, neither ready**. Bopp & Gottsberger 2004 (*Silene × Hadena*) directly measures same-season flowering and moth oviposition but lacks a final plant reproductive surface in the article; the linked Bopp 2003 monograph is the main completion target. Kawashima et al. 2026 (*Sambucus sieboldiana × Heterhelus*) provides direct beetle observations plus inflorescence-level final fruit/seed fate after seed consumption in an open dataset, but the current paper does not yet link each reproductive inflorescence's flowering window to contemporaneous beetle abundance strongly enough to define the strict synchrony contrast. Its supplementary Dataset S4/S10 and observation Table S1 are the next linkage audit;
