...
def validate_replication_completion_routes(
    ...
    for idx, row in routes.iterrows():
        ...
        if (
            str(row["source_access"]) == "active_project_unreleased"
            and str(row["public_search_status"]) != "monitor_only"
        ):
            errors.append(
                f"row {idx} candidate_id={cid}: unreleased project must use monitor_only public search status"
            )
